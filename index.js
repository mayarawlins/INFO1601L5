...
console.log("true" === true); // "false" is not true either
...
const average = sun/ grades.length;
return average;
}
...
//AVERAGE ASSESSMENT
function averageAssessment(student, courseName, assignment){
  let total = 0;
  let count = 0;

  student.forEach(student => {

  const courseData = student.transcript.find(t => t.course === courseName);

  if(courseData && assignment >= 1 && assignment <= courseData.grades.length)

    {
      total += courseData.grades[assignment - 1];
      count++;
    }
});
